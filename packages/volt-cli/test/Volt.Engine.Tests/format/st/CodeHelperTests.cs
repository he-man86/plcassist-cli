...
/// <summary>ParseCodeHeader classifies an item from its DECLARATION using the leading KEYWORD alone — it
/// requires nothing after the name on the header line, so a `: type` / `EXTENDS Base :` tail that legally wraps
/// onto the next line (a real CODESYS export form) can never make a header unrecognizable. The bug that
/// motivated that: pro2193's Fanuc_* structs wrapped `EXTENDS … :` and were silently dropped from the pull
/// (9 items).
///
/// <para>It no longer returns a NAME. Nothing read one — an item's name is its FILENAME — and the value was
/// wrong wherever a modifier stood where the name was expected (`FUNCTION_BLOCK ABSTRACT libObject` answered
/// `ABSTRACT`, on 78 corpus files). Every header SHAPE below is kept; only the second half of each assertion is
/// gone, because there is no longer a second half to be wrong.</para></summary>
public class CodeHelperTests
{
    private static string Parse(string src) => CodeHelper.ParseCodeHeader(src);
    ...
    public void A_program_declared_after_an_inline_comment_is_a_program_not_a_function_block() =>
        Assert.Equal("program", Parse("(* the main task *) PROGRAM P\nVAR\nEND_VAR"));
    ...
    {
        Assert.Equal("property", Parse("PROPERTY Position\n: INT\n"));
    }

    [Fact]
    public void Function_with_wrapped_return_type_is_a_function()
    {
        Assert.Equal("function", Parse("FUNCTION Compute\n: REAL\nVAR_INPUT\n x : INT;\nEND_VAR"));
    }

    [Fact]
    public void Method_with_wrapped_return_type_is_a_method()
    {
        Assert.Equal("method", Parse("METHOD Step\n: BOOL\nVAR\nEND_VAR"));
    }

    [Fact]
    public void Enum_with_wrapped_colon_is_a_dut()
    {
        Assert.Equal("dut", Parse("TYPE Color\n: (Red, Green, Blue);\nEND_TYPE"));
    }
    ...
    [InlineData("INTERFACE IThing EXTENDS IBase", "interface")]
    [InlineData("FUNCTION Add : INT\nVAR_INPUT\nEND_VAR", "function")]
    [InlineData("ACTION DoWork", "action")]
    [InlineData("METHOD PUBLIC FINAL Run : BOOL", "method")]
    [InlineData("PROPERTY PROTECTED Speed : REAL", "property")]
    [InlineData("TYPE Handle : __XWORD; END_TYPE", "dut")]
    [InlineData("TYPE U : UNION\n a : INT;\nEND_UNION\nEND_TYPE", "dut")]
    [InlineData("VAR_GLOBAL\n g : INT;\nEND_VAR", "gvl")]
    public void Same_line_and_modifier_headers_parse(string src, string kind) =>
        Assert.Equal(kind, Parse(src));

    /// <summary>`FUNCTION_BLOCK` is not a `FUNCTION`. This used to depend on the ORDER of two regexes and said
    /// so in a comment; the keyword is compared as a whole token now, so the hazard is gone rather than avoided
    /// — but the case stays, because the guarantee is what matters, not how it is met.</summary>
    [Fact]
    public void FunctionBlock_is_not_misread_as_a_function()
    {
    ...
    {
        Assert.Equal("function_block", Parse("(* doc *)\n// note\nFUNCTION_BLOCK FB\nVAR\nEND_VAR"));
    }
    ...
    public void HeaderLine_and_ParseCodeHeader_agree_on_which_line_is_the_header()
    {
        ...
        Assert.Equal("program", Parse(decl));
    }
}
