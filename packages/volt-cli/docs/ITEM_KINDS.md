...
| 621 | `PLCTASK` | `PlcTask` | `task` | both | ✅ | **rw (both)** | ✅ | CODESYS `.task` body = scheduling descriptor (Type/Interval/Priority/Event/Watchdog/called-POUs) via `ScriptTaskObject`, and the FIRST non-source kind a push may write: create, update, rename and delete all round-trip (`test/e2e/items/task-writable.test.ts`). The format is shared (`Volt.Engine/Format/Task/TaskDescriptorFormat`) so read and write cannot drift, and a non-canonical body is refused with the exact text to use. Writable ≠ SOURCE: it is not ST, the LSP does not parse it, and it stays out of the four `SOURCE_EXTENSIONS` manifests. TwinCAT renders and writes the SAME descriptor: it assembles one from two tree items (the PLC task's `LinkedTask` names the SYSTEM task holding priority + cycle time; the ordered POU calls are the PLC item's own 650 children), and every write is READ BACK because this vendor accepts a schedule it does not apply. Three fields have no TwinCAT counterpart - a non-`Cyclic` type, an `Event:`, a watchdog - and are REFUSED rather than dropped. Both vendors' write support is DECLARED and gated (`test/Volt.Repo.Gates/VendorCapabilityParityTests.cs`), so a future one-sided kind cannot arrive unnoticed. See DIALECT C19 (CODESYS traps) and C19b (the TwinCAT two-item shape) |
