...
| 621 | `PLCTASK` | `PlcTask` | `task` | both | ✅ | **rw (CODESYS)** / read-only (TC) | ✅ | CODESYS `.task` body = scheduling descriptor (Type/Interval/Priority/Event/Watchdog/called-POUs) via `ScriptTaskObject`, and the FIRST non-source kind a push may write: create, update, rename and delete all round-trip (`test/e2e/items/task-writable.test.ts`). The format is shared (`Volt.Engine/Format/Task/TaskDescriptorFormat`) so read and write cannot drift, and a non-canonical body is refused with the exact text to use. Writable ≠ SOURCE: it is not ST, the LSP does not parse it, and it stays out of the four `SOURCE_EXTENSIONS` manifests. TC still emits `Name=/linked-task=` (the parity gap) and its driver REFUSES the write with a reason. That one-sided capability is DECLARED and gated (`test/Volt.Repo.Gates/VendorCapabilityParityTests.cs`), so it cannot rot quietly and cannot be joined by another unnoticed; closing it is one measurement, written up in `scripts/probe-tc-task-write.md`. See DIALECT C19 for the vendor traps |
